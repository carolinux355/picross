// This is a generated file - do not edit.
//
// Generated from Configuration/BaseMessage.proto.

// @dart = 3.3

// ignore_for_file: annotate_overrides, camel_case_types, comment_references
// ignore_for_file: constant_identifier_names
// ignore_for_file: curly_braces_in_flow_control_structures
// ignore_for_file: deprecated_member_use_from_same_package, library_prefixes
// ignore_for_file: non_constant_identifier_names, prefer_relative_imports

import 'dart:core' as $core;

import 'package:protobuf/protobuf.dart' as $pb;

import 'ComponentSet.pb.dart' as $0;

export 'package:protobuf/protobuf.dart' show GeneratedMessageGenericExtensions;

class BaseMessage extends $pb.GeneratedMessage {
  factory BaseMessage({
    $core.String? id,
    $0.ComponentSet? components,
  }) {
    final result = create();
    if (id != null) result.id = id;
    if (components != null) result.components = components;
    return result;
  }

  BaseMessage._();

  factory BaseMessage.fromBuffer($core.List<$core.int> data,
          [$pb.ExtensionRegistry registry = $pb.ExtensionRegistry.EMPTY]) =>
      create()..mergeFromBuffer(data, registry);
  factory BaseMessage.fromJson($core.String json,
          [$pb.ExtensionRegistry registry = $pb.ExtensionRegistry.EMPTY]) =>
      create()..mergeFromJson(json, registry);

  static final $pb.BuilderInfo _i = $pb.BuilderInfo(
      _omitMessageNames ? '' : 'BaseMessage',
      createEmptyInstance: create)
    ..aOS(1, _omitFieldNames ? '' : 'id')
    ..aOM<$0.ComponentSet>(2, _omitFieldNames ? '' : 'components',
        subBuilder: $0.ComponentSet.create)
    ..hasRequiredFields = false;

  @$core.Deprecated('See https://github.com/google/protobuf.dart/issues/998.')
  BaseMessage clone() => deepCopy();
  @$core.Deprecated('See https://github.com/google/protobuf.dart/issues/998.')
  BaseMessage copyWith(void Function(BaseMessage) updates) =>
      super.copyWith((message) => updates(message as BaseMessage))
          as BaseMessage;

  @$core.override
  $pb.BuilderInfo get info_ => _i;

  @$core.pragma('dart2js:noInline')
  static BaseMessage create() => BaseMessage._();
  @$core.override
  BaseMessage createEmptyInstance() => create();
  @$core.pragma('dart2js:noInline')
  static BaseMessage getDefault() => _defaultInstance ??=
      $pb.GeneratedMessage.$_defaultFor<BaseMessage>(create);
  static BaseMessage? _defaultInstance;

  @$pb.TagNumber(1)
  $core.String get id => $_getSZ(0);
  @$pb.TagNumber(1)
  set id($core.String value) => $_setString(0, value);
  @$pb.TagNumber(1)
  $core.bool hasId() => $_has(0);
  @$pb.TagNumber(1)
  void clearId() => $_clearField(1);

  @$pb.TagNumber(2)
  $0.ComponentSet get components => $_getN(1);
  @$pb.TagNumber(2)
  set components($0.ComponentSet value) => $_setField(2, value);
  @$pb.TagNumber(2)
  $core.bool hasComponents() => $_has(1);
  @$pb.TagNumber(2)
  void clearComponents() => $_clearField(2);
  @$pb.TagNumber(2)
  $0.ComponentSet ensureComponents() => $_ensure(1);
}

const $core.bool _omitFieldNames =
    $core.bool.fromEnvironment('protobuf.omit_field_names');
const $core.bool _omitMessageNames =
    $core.bool.fromEnvironment('protobuf.omit_message_names');
